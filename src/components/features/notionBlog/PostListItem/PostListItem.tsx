...
export const PostListItem: FC<Props> = ({ post }) => {
  ...
  return (
    ...
    >
      ...
      />
      <div /*icon+tag+Updated,Categoy+Title*/className="flex-col justify-between w-full">
        <div /*icon+tag+Updated,Categoy*/className="flex justify-between w-full">
          <div /*icon+tag*/ className="flex items-center">
            <div /*icon*/ className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-full bg-white shadow sp:absolute sp:-top-2 sp:-left-2 sp:h-9 sp:w-9">
          ...
          <p /*Updated,Category*/ className="whitespace-nowrap text-sm justify-between font-bold sp:absolute sp:top-1.5 sp:right-2 sp:text-xs">
            <div>{meta.updatedAt}</div>
            <div className="font-bold">{meta.category.name}</div>
          </p>
        </div>
        <h3 className="font-bold line-clamp-2 sp:text-base mt-4">{meta.title}</h3>
      </div>
    </div>
  );
};
